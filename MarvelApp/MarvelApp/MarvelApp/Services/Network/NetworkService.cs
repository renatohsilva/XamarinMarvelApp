...
namespace MarvelApp.Services.Network
{
    public class NetworkService : INetworkService
    {
        ...
        {
            return await WaitAndRetryInner(func, sleepDurationProvider);
        }

        public async Task<T> WaitAndRetry<T>(Func<Task<T>> func, Func<int, TimeSpan> sleepDurationProvider, int retryCount)
        {
            return await WaitAndRetryInner(func, sleepDurationProvider, retryCount);
        }
